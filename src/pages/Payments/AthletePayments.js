      ...
      payments_data.forEach((id)=>{
        if(id.status == "paid"){
          completed.push(
            ...
            <div >
              <p style={{fontSize:16,color:"black",margin:0}}>{id.athleteName}</p>
                <p style={{fontSize:16,color:"black",margin:0}}>{moment(id.date.seconds*1000).format("ll")}</p>
            </div>
          ...
          if(moment(moment(new Date()).format("DD-MM-YYYY"),"DD-MM-YYYY").valueOf() <= id.date.seconds*1000 && moment(moment(new Date()).add(1,"days").format("DD-MM-YYYY"),"DD-MM-YYYY").valueOf() >= id.date.seconds*1000 ){
            today.push(
              <div key={id.id} style={{display:"flex",padding:10,marginBottom:15,backgroundColor:"white",paddingRight:20,paddingLeft:20,borderRadius:10,justifyContent:"space-between",alignItems:"center"}}>
                  <div >
                    <p style={{fontSize:16,color:"black",margin:0}}>{id.athleteName}</p>
                      <p style={{fontSize:16,color:"black",margin:0}}><b>Due on:</b> {moment(id.date.seconds*1000).format("ll")}</p>
                  </div>
                  <div style={{margin:0,padding:0}}>
                    <p style={{fontSize:16,color:"black",margin:0,padding:0,textAlign:"center"}}>{"\u20B9"} {id.amt}</p>
                    <div style={{backgroundColor:"#fcd54a",padding:5,cursor:"pointer",marginTop:2,borderRadius:5}}>
                      Pay Now
                    </div>
                  </div>
              </div>
